window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var css = [
      ...
      // Token activity monitor: one dot per day, a week per column, the whole
      // 53-week window filling the pane. Cell size is set from JS
      // (--dtt-dot / --dtt-gap on the scroller) because CSS alone cannot keep a
      // fixed 54-column grid and still know how wide the settings pane is; the
      // chips re-read the same grid (own day / its week / running total).
      ".dtt-act{flex-direction:column;gap:8px;display:flex}",
      ".dtt-actScroll{overflow-x:auto;padding-bottom:2px}",
      ".dtt-actBody{gap:6px;display:flex;min-width:0}",
      ".dtt-actDays{width:24px;flex:0 0 auto;grid-template-rows:repeat(7,var(--dtt-dot,9px));gap:var(--dtt-gap,4px);font-size:9.5px;color:var(--dsw-alias-label-secondary);display:grid}",
      ".dtt-actDays span{align-self:center;line-height:1}",
      ".dtt-actWeeks{gap:var(--dtt-gap,4px);display:flex;min-width:0}",
      ".dtt-actWeek{flex:0 0 auto;grid-template-columns:var(--dtt-dot,9px);grid-template-rows:repeat(7,var(--dtt-dot,9px));gap:var(--dtt-gap,4px);display:grid}",
      ".dtt-actMonths{margin-top:4px;padding-left:30px;gap:var(--dtt-gap,4px);font-size:10.5px;color:var(--dsw-alias-label-secondary);display:flex}",
      ".dtt-actMonth{flex:0 0 var(--dtt-dot,9px);min-width:0;text-align:left;white-space:nowrap;overflow:visible}",
      // Empty days need their own wash: the panel and the module-platform token
      // are the same colour in the app's dark theme, so a bare fill would make
      // the whole grid vanish. border-l3 flips with the theme (white 16 % on
      // dark, black 12 % on light).
      ".dtt-dot{width:100%;height:100%;border-radius:50%;background:var(--dsw-alias-border-l3)}",
      ".dtt-dot[data-first='true']{outline:1.5px solid var(--dsw-alias-label-primary);outline-offset:1.5px}",
      ".dtt-calCaption{align-items:center;flex-wrap:wrap;gap:8px;font-size:11.5px;color:var(--dsw-alias-label-secondary);display:flex}",
      ".dtt-calCaptionSpacer{flex:1 1 auto}",
      ".dtt-calMarker{width:11px;height:11px;flex:0 0 auto;border-radius:50%;box-shadow:inset 0 0 0 1.5px var(--dsw-alias-label-primary);display:inline-block}",
      ...
      ".dtt-legendCells{gap:4px;align-items:center;display:inline-flex}",
      ".dtt-legendCells .dtt-dot{flex:0 0 auto;width:11px}"
    ].join("");
    ...
    var ACTIVITY_SUBS = {
      daily: "shade = that day's tokens",
      weekly: "shade = the week's total",
      cumulative: "shade = running total"
    };
    ...
    /** Columns (weeks) the fixed window needs — the same walk the grid does. */
    function activityColumns(calendar) {
      if (calendar === undefined || calendar === null || calendar.start === undefined) return 0;
      var start = new Date(calendar.start);
      var end = new Date(calendar.end);
      var first = new Date(start.getFullYear(), start.getMonth(), start.getDate());
      var last = new Date(end.getFullYear(), end.getMonth(), end.getDate());
      var span = Math.round((last - first) / 86400000) + 1;
      return Math.ceil((span + first.getDay()) / 7);
    }

    /**
     * Token activity monitor: one dot per day, weeks as columns, the whole
     * fixed 53-week window filling the panel width — GitHub's grid, drawn as
     * circles with the month rail under it.
     *
     * The mode chips re-read the same grid: Daily is each day's own tokens,
     * Weekly the day's week total (so a column lights as one unit), Cumulative
     * the running total through that day (a ramp that shows how the year paces).
     * Levels are sqrt-scaled so one 375M-token day cannot flatten every other
     * day to the first step.
     */
    function TokenActivity(props) {
      var calendar = props.calendar;
      var mode = props.mode === undefined || props.mode === null ? "daily" : props.mode;
      var streaks = props.streaks === undefined || props.streaks === null ? {} : props.streaks;
      // Dot size is measured, not guessed: a fixed 54-column grid has to know the
      // pane width to fill it exactly, and the settings pane is resizable.
      var scrollRef = react.useRef(null);
      var geomState = react.useState({ dot: 9, gap: 4 });
      var geom = geomState[0];
      var setGeom = geomState[1];
      var columns = activityColumns(calendar);
      react.useEffect(function () {
        var node = scrollRef.current;
        if (node === null || columns <= 0) return undefined;
        var RAIL = 30; // weekday rail (24px) plus its gap
        var LABEL = 14; // overhang of the trailing month label
        var measure = function () {
          var room = node.clientWidth - RAIL - LABEL;
          if (room <= 0) return;
          var gap = 4;
          var dot = Math.floor((room - gap * (columns - 1)) / columns);
          if (dot < 7) {
            gap = 3;
            dot = Math.floor((room - gap * (columns - 1)) / columns);
          }
          if (dot < 6) {
            gap = 2;
            dot = Math.floor((room - gap * (columns - 1)) / columns);
          }
          setGeom({ dot: Math.max(3, Math.min(13, dot)), gap: gap });
        };
        measure();
        if (typeof ResizeObserver !== "function") {
          // No observer (old browser, or a test sandbox): a resize listener is
          // the fallback, and an environment without one keeps the initial size.
          try {
        ...
        var observer = new ResizeObserver(function () { measure(); });
      ...
      // before the first session stay empty dots.
      var days = [];
      ...
      for (var index = 0; index < days.length; index += 1) {
        // Each day keeps its own weekday row: the days before this week's first
        // cell (and after its last) stay holes, so a column never shifts up.
        var slot = Math.floor((index + offset) / 7);
        var row = (index + offset) % 7;
        if (weeks[slot] === undefined) weeks[slot] = [];
        weeks[slot][row] = days[index];
      }
      // Week roll-ups: only Weekly reads them, but they keep the tooltip honest
      // about what a lit column means.
      var weekStats = weeks.map(function (week) {
        var stat = { tokens: 0, sessions: 0, steps: 0, costUsd: 0, priced: 0, start: null };
        for (var row = 0; row < week.length; row += 1) {
          var entry = week[row];
          if (entry === undefined || entry.sessions === 0) continue;
          stat.tokens += entry.tokens;
          stat.sessions += entry.sessions;
          stat.steps += entry.steps;
          if (entry.costUsd !== null && entry.costUsd !== undefined) {
            stat.costUsd += entry.costUsd;
            stat.priced += 1;
          }
          if (stat.start === null) stat.start = entry.day;
        }
        stat.costUsd = stat.priced === 0 ? null : stat.costUsd;
        return stat;
      });
      var running = 0;
      var values = days.map(function (entry, index) {
        if (mode === "weekly") return weekStats[Math.floor((index + offset) / 7)].tokens;
        if (mode === "cumulative") {
          if (entry.sessions > 0) running += entry.tokens;
          return running;
        }
        return entry.sessions > 0 ? entry.tokens : 0;
      });
      var peak = values.reduce(function (top, value) { return value > top ? value : top; }, 0);
      var fill = [
        null,
      ...
      var levelOf = function (value) {
        // Numbers only: a hole or a NaN must read as empty, never as a peak.
        if (!(value > 0) || !(peak > 0)) return 0;
        var share = Math.sqrt(value / peak);
        if (share <= 0.25) return 1;
        if (share <= 0.5) return 2;
        if (share <= 0.75) return 3;
        return 4;
      };
      var titleOf = function (entry, value, index) {
        var weekday = CAL_WEEKDAYS[parseDayKey(entry.day).getDay()];
        if (mode === "weekly") {
          var week = weekStats[Math.floor((index + offset) / 7)];
          if (week.sessions === 0) return "week of " + fmtDay(entry.day) + " — no activity";
          var weekCost = week.costUsd === null ? "" : " · " + fmtUsd(week.costUsd);
          return "week of " + fmtDay(week.start) + " — " + fmtTokens(week.tokens) + " tokens · " + fmtInt(week.sessions) + " session(s) · " + fmtInt(week.steps) + " steps" + weekCost;
        }
        if (mode === "cumulative") {
          return weekday + " " + fmtDay(entry.day) + " — " + fmtTokens(value) + " tokens spent through this day";
        }
        if (entry.sessions === 0) return weekday + " " + fmtDay(entry.day) + " — no activity";
        var cost = entry.costUsd === null || entry.costUsd === undefined ? "" : " · " + fmtUsd(entry.costUsd);
        var marker = entry.day === firstKey ? " · first session" : "";
        return weekday + " " + fmtDay(entry.day) + " — " + fmtTokens(entry.tokens) + " tokens · " + fmtInt(entry.sessions) + " session(s) · " + fmtInt(entry.steps) + " steps" + cost + marker;
      };

      var seenMonth = null;
      var weekAnchor = function (week) {
        for (var row = 0; row < 7; row += 1) {
          if (week[row] !== undefined) return week[row].day;
        }
        return null;
      };
      var monthCells = weeks.map(function (week, weekIndex) {
        var anchor = weekAnchor(week);
        if (anchor === null) return h("span", { className: "dtt-actMonth", key: String(weekIndex) }, "");
        var first = parseDayKey(anchor);
        if (seenMonth === null || first.getMonth() !== seenMonth) {
          seenMonth = first.getMonth();
          return h("span", { className: "dtt-actMonth", key: String(weekIndex) }, CAL_MONTHS[seenMonth]);
        }
        return h("span", { className: "dtt-actMonth", key: String(weekIndex) }, "");
      });
      ...
      return h("div", { className: "dtt-act" }, [
        h("div", {
          className: "dtt-actScroll",
          key: "grid",
          ref: scrollRef,
          style: { "--dtt-dot": geom.dot + "px", "--dtt-gap": geom.gap + "px" }
        }, [
          h("div", { className: "dtt-actBody", key: "body" }, [
            h("div", { className: "dtt-actDays", key: "days" }, ["", "Mon", "", "Wed", "", "Fri", ""].map(function (label, row) {
              return h("span", { key: String(row) }, label);
            })),
            h("div", { className: "dtt-actWeeks", key: "weeks" }, weeks.map(function (week, weekIndex) {
              return h("div", { className: "dtt-actWeek", key: String(weekIndex) }, [0, 1, 2, 3, 4, 5, 6].map(function (row) {
                var entry = week[row];
                if (entry === undefined) return h("div", { className: "dtt-dot", key: String(row), "data-level": "0", "data-empty": "true" });
                var index = weekIndex * 7 + row - offset;
                var value = values[index];
                ...
                return h("div", {
                  ...
                  title: titleOf(entry, value, index)
                });
              }));
            }))
          ]),
          h("div", { className: "dtt-actMonths", key: "months" }, monthCells)
        ]),
        h("div", { className: "dtt-calCaption", key: "caption" }, [
          h("span", { className: "dtt-calMarker", key: "marker" }),
        ...
        h("div", { className: "dtt-calFoot", key: "foot" }, [
          ...
          h("div", { className: "dtt-legendCells", key: "legend" }, [
            ...
            [0, 1, 2, 3, 4].map(function (level) {
              return h("span", { className: "dtt-dot", key: String(level), "data-level": String(level), style: level === 0 ? undefined : { backgroundColor: fill[level] } });
            }),
    ...
    function TokenTrackerSection() {
      ...
      var activityState = react.useState("daily");
      var activityMode = activityState[0];
      var setActivityMode = activityState[1];
      ...
      var activityPanel = h(Panel, {
        key: "activity",
        title: "Token activity",
        sub: ACTIVITY_SUBS[activityMode],
        actions: h("div", { className: "dtt-chips", "data-dtt": "activity-modes" }, ACTIVITY_MODES.map(function (entry) {
          return h(Chip, { key: entry.key, label: entry.label, on: entry.key === activityMode, onClick: function () { setActivityMode(entry.key); } });
        }))
      }, h(TokenActivity, { calendar: data.calendar, streaks: streaks, mode: activityMode }));
