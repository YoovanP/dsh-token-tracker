...
function buildCalendar(records, now) {
  ...
  const floor = startOfDay(now - (CALENDAR_DAYS - 1) * DAY_MS)
  const start = oldest === null ? end : Math.max(floor, startOfDay(oldest))
  const days = []
  let cursor = new Date(start)
  while (cursor.getTime() <= end) {
    const key = dayKey(cursor.getTime())
    const entry = daily.get(key)
    days.push(entry === undefined
      ? { day: key, sessions: 0, tokens: 0, billableTokens: 0, cacheReadTokens: 0, steps: 0, turns: 0, costUsd: null }
      : { sessions: entry.sessions, tokens: entry.tokens, billableTokens: entry.billableTokens, cacheReadTokens: entry.cacheReadTokens, steps: entry.steps, turns: entry.turns, day: key, costUsd: entry.priced === 0 ? null : entry.costUsd })
  ...
  for (let index = days.length - 1; index >= 0; index -= 1) {
    if (days[index].sessions > 0) current += 1
    else break
  }
  const activeDays = days.filter((entry) => entry.sessions > 0).length
  const best = days.reduce((winner, entry) => (winner === null || entry.tokens > winner.tokens ? entry : winner), null)
  return {
    start,
    end,
    days,
    streaks: {
      ...
      bestDay: best === null ? null : { day: best.day, tokens: best.tokens, sessions: best.sessions },
    },
  }
}
